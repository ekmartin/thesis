...
This thesis implements support for log based recovery in Soup, utilizing
the system's existing log infrastructure. It then primarily focuses on snapshot
recovery, investigating different options for snapshotting, until finally
...
\begin{figure}
  ...
  \caption{
    Soup (here: Phloem) outperforms even the popular caching
...
\begin{figure}
  ...
  \caption{
    Soup (here: Phloem) still holds up well when faced with a 50\%-50\% mixed
    workload.
  }
\end{figure}
...
keep track of the last successfully completed snapshot.
...
A domain can be uniquely identified by its domain and shard index. The domain
...
Each node includes a local address unique for the domain it belongs to. As
...
throughput of snapshotting every 10th second. The results here are similar to
...
\section{Future work}
While the snapshotting solution presented in this thesis serves well as an
...
the snapshot workers to the controller: it could simply check the same remote
file system to see if each domain's snapshot was completed.
...
current system also stores each node's log locally.

\subsection{Log Recovery Improvements}
Although this thesis has had a focus on snapshot recovery so far, regular
replaying of log entries is an important part of the recovery process in Soup.
The current implementation serializes log entries using JSON, as described in
section \ref{sec:logging}, and a more efficient serialization format might
perform better.
...
\section{Conclusion}
Throughout this thesis we have described the internals of the Soup database
system, and how its query graph affects recovery options. We have implemented
a baseline option for recovery, log based replay, and looked at how it can be
improved using snapshotting. After defining a set of requirements for
snapshotting, we designed a Soup compatible snapshotting method inspired by
Chandy-Lamport's algorithm for snapshotting in distributed systems.

Following that, we implemented snapshotting in the current Soup system, and
benchmarked the result it had on our system's recovery time. The initial
findings where positive, with a 90\% reduction in recovery time compared to only
log recovery. We then looked at the run-time impact snapshotting had on Soup, by
benchmarking write throughput with and without periodic snapshots. The results
so far show that the penalty is insignificant.

Finally, we looked at possible next iterations of snapshotting in Soup. While
the method implemented here is a good first iteration, there is without doubt
work left until we reach a production level snapshotting system in Soup.
