...
\author{Lars Martin Bævre Ek \\ \texttt{larek@stud.ntnu.no}}
\date{\today}
\maketitle
...
\begin{abstract}
  ...
  Soup achieves this by building up and maintaining a data-flow graph based on
  the queries the system needs to handle, where updates are propagated through
  the graph in a streaming manner. State is materialized along the graph
  incrementally, moving the bulk of the query processing work from write-time to
  read-time. By adapting the graph to schema changes on-the-fly, Soup avoids the
  substantial downtime that comes with migrations in traditional relational
  ...
  Recovery is based on replaying semantic updates from log entries, which
  ...
  node in a distributed setting. Initial results are promising, and shows Soup
  recovering in about a tenth of the regular recovery time when using snapshots,
...
across multiple machine to be able to work with bigger datasets.

Being able to distribute domains across a network is a good first step towards
this, but what happens when a single domain is too big for one machine's memory?
This is the case if we consider a single node in the query graph to be the
smallest atomic unit in a Soup system. With sharding however, it is not.
Sharding lets Soup divide up the work normally done by a single domain into
multiple domain shards, which can then be distributed across its instances as it
sees fit.
...
consideration for the previous or next write inserted into the system.
...
\textit{WHERE} clauses result in filter operators.
...
of an SQL \textit{ORDER BY} clause, together with a \textit{LIMIT}. In a similar
