...
failure occuring after that domain has snapshotted, but before the entire graph
...
Snapshot recovery comes in addittion to the log based recovery described in
...
to be restored at each materialized node across the graph - not solelyat the base
...
log based recovery. The same logic also holds for the intitial recovery time: if
...
\ref{sec:vote}, but instead of measuring the time it takes to recover, we benchmark
the total amount of writes our database is able to handle per second: the write
throughput. This is measured using different snapshot timeouts \footnote{The
amount of time the controller waits before starting a new snapshot.}, and
compared to the baseline of using no snapshotting at all. Unless otherwise
specified, Soup runs with two worker threads, a single client thread and each
domain is sharded into two separate shards.
...
To simulate a situation where that is not the case, where the very existance of
the snapshotting worker would cause a performance hit to the rest of the system,
we increase the amount of client threads writing to our database in parallel.
