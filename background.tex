...
\begin{figure}[H]
  \centering
  \includesvg[width=0.8\textwidth]{data-flow}
  \caption{\
...
\begin{figure}[H]
  \centering
  \includesvg[width=\textwidth]{replay}
  \caption{\
    Queries for missing keys, in this case \code{X}, are fulfilled by requesting
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.5\textwidth]{migrate}
  \caption{\
    To extend the program from figure~\ref{fig:data-flow} with a
...
Soup is composed of a series of components and runs distributed on an arbitrary
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.8\textwidth]{architecture}
  \caption{\
...
node---either \code{Article} or \code{Vote}---before they propagate through the
graph. Both base nodes are fully materialized---they serve as the source of
truth for all data inserted into Soup. Continuing, we arrive at the egress and
ingress nodes shown on level \textbf{II}. These act as connectors between
separate domains, and facilitate communication either within a single Souplet or
across machines if Soup is distributed across a cluster.

\begin{figure}[H]
  \centering
  \includesvg[width=0.8\textwidth]{example-graph}
  \caption{\
    The data-flow graph produced by Soup for the example program shown in
    listing~\ref{lst:soup-schema}. Nodes are marked with either \code{(full)} or
    \code{(part.)}, depending on whether their state is fully or partially
    materialized.
  }\label{fig:example-graph}
\end{figure}

The third label, \textbf{III}, shows the first operator in the data-flow graph,
an aggregator. Described by the gamma symbol ($ \gamma $), it performs the work
of our \code{COUNT(uid)} clause. The aggregation is incrementally maintained,
...
Moving along, we reach the join operator in our data-flow graph, responsible for
combining rows from the \code{Article} and the \code{Vote} tables. Notice that
the node is neither partially nor fully materialized---it contains no state at
all. Whenever an update arrives from one of its parents, it requests the rows
necessary to perform a join from the other parent. Similar to the aggregation,
the join node is also followed by a projection, which in this case simply emits
all the fields from the join.

Finally, we reach the reader node at the bottom of the graph labeled with the
external query it serves requests for: \code{ArticleWithVoteCount}. While
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{shim}
  \caption{\code{distributary-mysql} translates SQL-queries to appropriate Soup
  API-calls.}
\end{figure}
