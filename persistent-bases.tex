...
What alternatives does this leave us? One option would be to only write to
durable storage from background threads, avoiding the hit in latency that comes
with writing to and reading from disk. Instead, writes to \code{PersistentState}
could fill up an in-memory queue structure, which would then later be flushed to
persistent storage by background threads. Lookups could then read from both the
in-memory queue and the slower---but persistent---SQLite. This is effectively
the structure of a log-structured merge-tree, described in
section~\ref{sec:rocksdb}, where writes are initially kept in an in-memory
buffer, and later flushed to disk by background workers. That means that instead
of re-inventing the wheel that is LSM-trees, we can embed a storage engine that
already makes use of such a data structure internally: RocksDB.
